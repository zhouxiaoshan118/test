...
<form id="add" class="layui-form layui-fluid layui-col-md12" action="" style="display:none;">
    <div style="width:700px;height:380px;display: block;margin: 9px auto;">
        ...
        <div class="layui-form-item layui-col-md10">
            ...
            <div class="layui-input-block">
                <input type="text" name="weight" maxlength="255" required lay-verify="required" placeholder="请输入权重" autocomplete="off" class="layui-input">
            </div>
        </div>
        <div class="layui-form-item layui-col-md8">
            <label class="layui-form-label">日报图片</label>
            <div class="layui-input-block">
...
<script type="text/javascript" >
    ...
    $(document).ready(function(){
        ...
        <#--打开后台分页-->
            ...
            ajax : function(data, callback, settings) {
                ...
                $.ajax({
                    type : "GET",
                    url : "${adminPath}/sys/dailyArticleChecked/select",
                    cache : false, //禁用缓存
                    data : data, //传入已封装的参数
                    dataType : "json",
            ...
            "columnDefs": [
...

                {
                    ...
                    "render" : function(data, type, row) {
                        var id = '"' + row.id + '"';
                        var html = "<button onclick='javascript:updateOp(this);' class='layui-btn  updateOpBtn layui-btn-xs' value="+id+">编辑</button>";
                        html += "<button  onclick='javascript:deleteOp(this);' class='layui-btn layui-btn-danger layui-btn-xs' value="+id+">删除</button>&nbsp;";
                        html += "<button  onclick='javascript:updateAudit(this);' class='layui-btn layui-btn-warm layui-btn-xs' value="+id+">通过审核</button>&nbsp;";
                        return html;
                    }
                }
    ...
    layui.use(['form','layer'] ,function() {
        ...
        form.on('submit(addSub)', function (data) {
            ...
            layer.confirm('您是否确认添加数据？', {
                ...
                $.ajax({
                    type:"post",
                    url: "${adminPath}/sys/dailyArticle",
        ...
        form.on('submit(updateSub)', function (data) {
            ...
            layer.confirm('您是否确定修改数据？', {
                ...
                $.ajax({
                    type:"put",
                    url: "${adminPath}/sys/dailyArticle/"+$("#dicid").attr("value"),
                    data: JSON.stringify(data.field),
                    contentType:"application/json",
                    success:function(res) {
    ...
    <#-- 更新操作 -->
        ...
        $.ajax({
            type:'get',
            url:'/admin/sys/dailyArticle/checkart/'+id,
            success:function(res) {
                layer.msg("审核完毕",{icon: 1}, function() {
                    dataTable.ajax.reload();
                });
            },
            error:function() {
    ...
    <#-- 删除操作 -->
        ...
        layer.confirm('您是否确定删除数据？', {
            ...
            $.ajax({
                type:"delete",
                url:"${adminPath}/sys/dailyArticle/"+id,
                success:function() {
